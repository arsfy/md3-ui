...
const Md3CardTitle = forwardRef<HTMLHeadingElement, HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn('text-base font-medium leading-6 tracking-wide text-[var(--md3-onSurface)]', className)}
      {...props}
    />
  ),
)
...
const Md3CardDescription = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p
      ref={ref}
      className={cn('text-sm leading-5 tracking-[0.25px] text-[var(--md3-onSurfaceVariant)] mt-1', className)}
      {...props}
    />
  ),
)
